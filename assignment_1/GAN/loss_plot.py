...
class LossPlot:
    ...
    def __init__(self,
        ...
        self.train_loss = []
        self.test_loss = []
        # self.line,        = self.ax.plot([], [], color='b', alpha=0.8)
        self.line_smooth, = self.ax.plot([], [], color='g', label='Discriminator loss')
        self.line_test,   = self.ax.plot([], [], color='r', label='Generator loss')
    ...
    def add_test_item(self, loss):
        ...
        if type == 'savgol':
            ...
            train_smooth = numpy_ewma_vectorized(self.test_loss, self.window)
            self.line_test.set_ydata(train_smooth)
            self.line_test.set_xdata(xdata)

        self.ax.set_ylim(0, max(self.test_loss))
        self.ax.set_xlim(0, self.epochs)
